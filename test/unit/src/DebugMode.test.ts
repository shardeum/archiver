...
describe('DebugMode', () => {
  ...
  describe('isDebugMiddleware', () => {
    ...
    it('should update lastCounter when verification succeeds', () => {
      // Use isolateModules to ensure a fresh lastCounter value and module state
      jest.isolateModules(() => {
        // When using isolateModules, we need to re-mock any dependencies
        // with the virtual: true option to ensure proper mocking
        jest.mock(
          '../../../src/Crypto',
          () => ({
            verify: jest.fn(() => true),
        ...
        // Re-import the modules to get a fresh instance with our mocks
        const Crypto = require('../../../src/Crypto')
        const { isDebugMiddleware } = require('../../../src/DebugMode')
