...
describe('AllowedArchiversManager', () => {
  ...
  describe('Initialization', () => {
    it('should initialize successfully with valid config path', () => {
      ...
      expect(fs.watchFile).toHaveBeenCalledWith(expect.any(String), { persistent: true }, expect.any(Function))
  ...
  describe('File Watching', () => {
    ...
    it('should reload config when file changes', () => {
      ...
      if (listener) {
        ...
        listener({ mtime: new Date(2022, 1, 1) }, { mtime: new Date(2021, 1, 1) })
    ...
    it('should not reload config when file mtime is unchanged', () => {
      ...
      if (listener) {
        ...
        listener({ mtime: sameTime }, { mtime: sameTime })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle config path with special characters', () => {
      ...
      expect(fs.watchFile).toHaveBeenCalledWith(specialPath, { persistent: true }, expect.any(Function))
    })
  ...
  describe('Integration Tests', () => {
    ...
    it('should maintain state consistency through multiple updates', () => {
      ...
      configs.forEach((config, index) => {
        ...
        const listener = (jest.mocked(fs.watchFile) as any)._listener
        listener({ mtime: new Date(2022, 1, index + 2) }, { mtime: new Date(2022, 1, index + 1) })
