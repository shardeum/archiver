...
describe('statistics/index', () => {
    ...
    beforeEach(() => {
      ...
      stats = new Statistics(
        ...
        mockConfig,
        {
          counters: [],
          watchers: { 
            cpuUsage: function() { return watcherValue },
            memoryUsage: function() { return this.appName === 'test-app' ? 100 : 200 }
          },
          timers: [],
          manualStats: [],
        },
  ...
  describe('timers', () => {
    ...
    it('should not duplicate timer start', () => {
      const now = Date.now()
      jest.spyOn(Date, 'now').mockReturnValueOnce(now).mockReturnValueOnce(now + 1000)
  ...
  describe('writeOnSnapshot', () => {
    ...
    it('should add custom write function', () => {
      ...
      const writeFn = function() { return `custom\t${this.value}\t${new Date().toISOString()}\n` }
  ...
  describe('average calculations', () => {
    ...
    it('should throw error for undefined ring holder', () => {
      ...
      expect(() => stats.getPreviousElement('nonexistent')).toThrow("Cannot read properties of undefined (reading 'ring')")
    })
  })
