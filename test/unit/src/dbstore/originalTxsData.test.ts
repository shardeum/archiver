...
jest.mock('../../../../src/dbstore/checkpointStatus', () => ({
  ...
  bulkUpdateCheckpointStatusField: jest.fn(() => Promise.resolve()),
}))
...
describe('OriginalTxsData Module', () => {
  ...
  describe('insertOriginalTxData', () => {
    ...
    it('should create checkpoint when both flags are true', async () => {
      ...
      expect(originalTxCheckpointManager.addData).toHaveBeenCalledWith(expect.any(Object), 'mock-bucket-id')
    ...
    // Test error handling
    it('should handle database error and log error message', async () => {
      const mockOriginalTxData = createMockOriginalTxData()
      const mockError = new Error('Database error')

      // Mock db.run to throw error
      jest.mocked(db.run).mockRejectedValue(mockError)

      // Mock Logger to spy on error calls
      const errorSpy = jest.spyOn(Logger.mainLogger, 'error')
    ...
    it('should log debug message when VERBOSE is true', async () => {
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('Successfully inserted OriginalTxData', mockOriginalTxData.txId)
    })
  })
  ...
  describe('bulkInsertOriginalTxsData', () => {
    ...
    it('should create checkpoints when both flags are true', async () => {
      // Set up test conditions
      config.checkpoint.bucketConfig.allowCheckpointUpdates = true
      const mockOriginalTxsData = [createMockOriginalTxData(), createMockOriginalTxData()]
    ...
    it('should handle database error and log error message', async () => {
      ...
      // Assert error was logged
      expect(errorSpy).toHaveBeenCalledWith(mockError)
      expect(errorSpy).toHaveBeenCalledWith('Unable to bulk insert OriginalTxsData', mockOriginalTxsData.length)
    })
    ...
    it('should log debug message when VERBOSE is true', async () => {
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('Successfully inserted OriginalTxsData', mockOriginalTxsData.length)
    })
  })
  ...
  describe('queryOriginalTxDataCount', () => {
    ...
    it('should log error when database throws', async () => {
      ...
      // Mock db.get to throw error
      jest.mocked(db.get).mockRejectedValue(mockError)

      // Mock Logger to spy on error calls
      const errorSpy = jest.spyOn(Logger.mainLogger, 'error')

      // The function will throw TypeError when trying to access undefined['COUNT(*)']
      // This is a bug in the implementation but we're testing current behavior
      await expect(originalTxsDataModule.queryOriginalTxDataCount()).rejects.toThrow(TypeError)
    ...
    it('should log debug message when VERBOSE is true', async () => {
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('OriginalTxData count', expect.anything())
      expect(count).toBe(42)
    })
  })
  ...
  describe('queryOriginalTxsData', () => {
    ...
    it('should log debug message when VERBOSE is true', async () => {
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('OriginalTxData originalTxsData', expect.any(Array))
      expect(result.length).toBe(2)
    })
  })
  ...
  describe('queryOriginalTxDataByTxId', () => {
    ...
    // Test verbose logging
    it('should log debug message when VERBOSE is true and record found', async () => {
      // Enable verbose mode
      config.VERBOSE = true
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('OriginalTxData txId', expect.any(Object))
  ...
  describe('queryOriginalTxDataCountByCycles', () => {
    ...
    it('should log debug message when VERBOSE is true', async () => {
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('OriginalTxData count by cycle', expect.any(Array))
      expect(result.length).toBe(2)
    })
  })
  ...
  describe('queryLatestOriginalTxs', () => {
    ...
    it('should log debug message when VERBOSE is true', async () => {
      ...
      // Assert debug was logged
      expect(debugSpy).toHaveBeenCalledWith('Latest Original-Tx: ', expect.any(Array))
      expect(result.length).toBe(2)
    })
  })
  ...
  describe('Edge Cases and Integration Tests', () => {
    it('should handle serialization of complex objects', async () => {
      const complexOriginalTxData = createMockOriginalTxData({
        originalTxData: {
          nested: {
            deep: {
              value: 'test',
      ...
      // Call the function
      await originalTxsDataModule.insertOriginalTxData(complexOriginalTxData, false)

      // Assert SerializeToJsonString was called with the complex object
      expect(SerializeToJsonString).toHaveBeenCalledWith(complexOriginalTxData.originalTxData)
    })

    it('should handle concurrent inserts gracefully', async () => {
      const txData1 = createMockOriginalTxData({ txId: 'tx1' })
      const txData2 = createMockOriginalTxData({ txId: 'tx2' })
      const txData3 = createMockOriginalTxData({ txId: 'tx3' })

      // Execute concurrent inserts
      await Promise.all([
        originalTxsDataModule.insertOriginalTxData(txData1, true),
        originalTxsDataModule.insertOriginalTxData(txData2, true),
        originalTxsDataModule.insertOriginalTxData(txData3, true),
      ])

      // Assert all inserts were attempted
      expect(db.run).toHaveBeenCalledTimes(3)
      expect(originalTxCheckpointManager.addData).toHaveBeenCalledTimes(3)
    })

    it('should handle State.isSyncing flag correctly in bulkInsert', async () => {
      // Set State.isSyncing to true
      ;(State as any).isSyncing = true
      ...
      // Assert checkpoint status was updated with isSyncing = true
      expect(checkpointStatus.bulkUpdateCheckpointStatusField).toHaveBeenCalledWith(
        CheckpointStatusType.ORIGINAL_TX,
        true, // isSyncing should be true
        undefined,
        undefined,
        expect.any(Array)
      )

      // Reset State.isSyncing
      ;(State as any).isSyncing = false
    })

    it('should handle boundary conditions for pagination', async () => {
      // Test with extreme skip/limit values
      await originalTxsDataModule.queryOriginalTxsData(Number.MAX_SAFE_INTEGER - 1, 1)

      // Assert query was executed
      expect(db.all).toHaveBeenCalledWith(
        expect.anything(),
        expect.stringContaining(`LIMIT 1 OFFSET ${Number.MAX_SAFE_INTEGER - 1}`),
        []
      )
    })

    it('should correctly transform database results', async () => {
      const mockDbResult = {
        txId: 'tx1',
        timestamp: 1000,
        cycle: 5,
        originalTxData: '{"nested":{"value":"test"}}',
      }

      jest.mocked(db.get).mockResolvedValue(mockDbResult)

      const result = await originalTxsDataModule.queryOriginalTxDataByTxId('tx1')

      // Assert the originalTxData was properly deserialized
      expect(result.originalTxData).toEqual({ nested: { value: 'test' } })
