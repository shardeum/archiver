...
jest.mock('sqlite3', () => {
  const mockDb = {
    ...
    all: jest.fn().mockImplementation((sql, params, callback) => {
      if (callback) callback(null, [{ id: 1, name: 'test' }, { id: 2, name: 'test2' }])
    }),
  ...
  describe('createDB', () => {
    ...
    it('should log slow and very slow queries', async () => {
      ...
      if (onCalls && onCalls.length > 0) {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith('VERY SLOW QUERY', expect.any(Number), 'INSERT INTO test VALUES (?)', 1200)
