...
describe('Gossip', () => {
  ...
  beforeEach(() => {
    ...
    // Setup spies
    postJsonSpy = jest.spyOn(P2P, 'postJson')
                      .mockImplementation(() => Promise.resolve({ success: true } as any));
    
    hashObjSpy = jest.spyOn(Crypto, 'hashObj')
                      .mockImplementation((obj) => JSON.stringify(obj));
    
    // Setup mock State
    jest.spyOn(State, 'getNodeInfo')
        .mockReturnValue({
    ...
    stateMetaDataMapGetSpy = jest.spyOn(StateMetaData.StateMetaDataMap, 'get')
                                 .mockImplementation(jest.fn());
    
    // Fix the return type to match required Promise<void>
    processStateMetaDataSpy = jest.spyOn(StateMetaData, 'processStateMetaData')
                                  .mockImplementation(() => Promise.resolve());
    
    jest.spyOn(StateMetaData, 'replaceDataSender')
        .mockImplementation(() => Promise.resolve());
  ...
  describe('sendGossip', () => {
    ...
    it('should not send gossip if there are no other archivers', async () => {
      ...
      expect(loggerDebugSpy).toHaveBeenCalledWith(
        'There is no other archivers to send our gossip'
      );
    });
  ...
  describe('convertStateMetadataToHashArray', () => {
    it('should convert state metadata to hash array', () => {
      const stateMetadata = {
        stateHashes: [
          { counter: 1, partitionHashes: {}, networkHash: 'hash1', receiptMapHashes: {}, networkReceiptHash: 'receipt1' },
          { counter: 2, partitionHashes: {}, networkHash: 'hash2', receiptMapHashes: {}, networkReceiptHash: 'receipt2' }
        ],
        receiptHashes: [
          { counter: 1, partitionHashes: {}, networkHash: 'hash3', receiptMapHashes: {}, networkReceiptHash: 'receipt3' },
          { counter: 3, partitionHashes: {}, networkHash: 'hash4', receiptMapHashes: {}, networkReceiptHash: 'receipt4' }
        ],
        summaryHashes: [
          { counter: 2, partitionHashes: {}, networkHash: 'hash5', receiptMapHashes: {}, networkReceiptHash: 'receipt5' },
          { counter: 3, partitionHashes: {}, networkHash: 'hash6', receiptMapHashes: {}, networkReceiptHash: 'receipt6' }
        ]
      };
  ...
  describe('Integration tests', () => {
    ...
    it('should handle gossip when all archivers agree', async () => {
      ...
      expect(Logger.mainLogger.error).not.toHaveBeenCalledWith(
        expect.stringContaining('our hash is different')
      );
