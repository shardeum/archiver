...
class AllowedArchiversManager {
  ...
  private validateConfig(config: any): boolean {
    return !!(config && Array.isArray(config.allowedArchivers) && Array.isArray(config.signatures))
  }
  ...
  public isArchiverAllowed(publicKey: string): boolean {
    ...
    return this.currentConfig.allowedArchivers.some((archiver) => archiver.publicKey === publicKey)
  }
}
