...
export const syncKeysFromNetworkConfig = async (): Promise<void> => {
  try {
    const queryFn = async (node): Promise<object> => {
      ...
      try {
        ...
        Logger.mainLogger.error(
          `Error querying node ${node.ip}:${node.port}: ${error} while updating dev public keys and multisig keys from /netconfig`
        )
        return null
      }
    }
    ...
    if (tallyItem?.value?.config?.debug) {
      ...
      if (
        typeof newMinMultiSigRequired === 'number' &&
        newMinMultiSigRequired > 0 &&
        newMinMultiSigRequired !== minMultiSigRequiredForGlobalTxs
      ) {
...
export const verifyTransaction = (tx: any): Response => {
  try {
    ...
    try {
      ...
      const { address: senderAddress, isValid: isSignatureValid } = getTxSenderAddress(transaction)
...
export function validateTransferFromSecureAccount(tx: any): { success: boolean; reason: string } {
  try {
    ...
    if (!isSignatureValid) {
      Logger.mainLogger.error(
        `Signature verification failed for secure account transfer transaction : ${StringUtils.safeStringify(tx)}`
      )
    ...
    Logger.mainLogger.error(
      `error in validation of secure account transaction verification ${StringUtils.safeStringify(tx)}`,
      ' and error is : ',
      error
    )
...
export function getTxSenderAddress(tx: TypedTransaction): GetTxSenderAddressResult {
  try {
