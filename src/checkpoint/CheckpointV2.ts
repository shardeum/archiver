...
export async function recordCheckpointStatus(
  ...
  try {
    ...
    const consensusPercentage =
      bucket.radixEntries.size > 0 ? radixesWithMajority / bucket.radixEntries.size : 0
    ...
    const status =
      matchedArchivers >= requiredMajority ? CheckpointSyncStatus.COMPLETED : CheckpointSyncStatus.FAILED

    // Record the checkpoint status
    const checkpointStatus: CheckpointStatus = {
      cycle,
      unifiedStatus: matchedArchivers >= requiredMajority ? true : false,
      cycleStatus: matchedArchivers >= requiredMajority ? true : false,
      receiptStatus: matchedArchivers >= requiredMajority ? true : false,
      originalTxStatus: matchedArchivers >= requiredMajority ? true : false,
      created_at: Date.now(),
    }
...
export async function syncMissingCheckpoints(maxCyclesToSync: number = 10): Promise<void> {
  try {
    ...
    for (let cycle = minCycle; cycle <= endCycle; cycle++) {
      ...
      if (!cycleStatus || cycleStatus.cycleStatus === false || cycleStatus.unifiedStatus === false) {
        ...
        try {
          ...
          const success = await Data.syncCycleData(cycle)
          if (success) {
            await updateCheckpointStatusField(cycle, CheckpointStatusType.CYCLE, true)
            ...
            await updateCheckpointStatusField(cycle, CheckpointStatusType.CYCLE, false)
            await updateCheckpointStatusField(cycle, CheckpointStatusType.RECEIPT, false)
            await updateCheckpointStatusField(cycle, CheckpointStatusType.ORIGINAL_TX, false)
          }
        } catch (error) {
