...
function handleFileError(err: Error): ApiError {
  ...
  return createApiError('TICKETS_FILE_NOT_ACCESSIBLE', `Unable to access tickets configuration: ${ticketFilePath}`)
}

function handleJsonParseError(err: Error): ApiError {
  Logger.mainLogger.error('Failed to parse tickets JSON:', err)
  return createApiError('INVALID_TICKETS_DATA', 'Invalid tickets configuration data')
}
...
function handleVerificationError(errors: VerificationError[]): ApiError {
  ...
  return createApiError('INVALID_TICKET_SIGNATURES', 'Ticket verification failed', errors)
}
...
export const ticketsRouter: FastifyPluginAsync = async function (fastify) {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        if (err.message === 'Invalid tickets format') {
          const error = createApiError('INVALID_TICKETS_FORMAT', 'Invalid tickets configuration format')
          return reply.code(error.statusCode).send(error.response)
        }
        if (err.message.includes('Ticket verification failed')) {
          const error = createApiError('INVALID_TICKET_SIGNATURES', 'Ticket verification failed')
          return reply.code(error.statusCode).send(error.response)
        }
      }

      const error = createApiError('INTERNAL_SERVER_ERROR', 'Internal server error')
      return reply.code(error.statusCode).send(error.response)
    }
  })
  ...
  const validateTicketType = (request: FastifyRequest): TicketType | ApiError => {
    ...
    if (!type || typeof type !== 'string') {
      return createApiError('INVALID_TICKET_TYPE', 'Invalid ticket type parameter')
    }
    return type
  }
  ...
  fastify.get('/:type', async (request, reply) => {
    ...
    try {
      ...
      if (!ticket) {
        const error = createApiError('TICKET_NOT_FOUND', `No ticket found with type: ${type}`)
        return reply.code(error.statusCode).send(error.response)
      }
