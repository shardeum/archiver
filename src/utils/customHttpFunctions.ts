import axios, {AxiosInstance, AxiosRequestConfig, AxiosProgressEvent, CancelTokenSource} from 'axios';
import {config} from "../Config";
import fetch, {RequestInfo, RequestInit, Response} from 'node-fetch'

// Add a custom interface to extend the config type
...
export function customAxios(maxBytes?: number, axiosConfig: AxiosRequestConfig = {}): AxiosInstance {
    ...
    instance.interceptors.request.use(request => {
        ...
        // Store the cancel source on the request
    ...
    instance.interceptors.response.use(
        response => {
            ...
            if (contentLength > 0 && contentLength > downloadLimit) {
                throw new Error(`Response content length ${contentLength} bytes exceeds limit of ${downloadLimit} bytes`);
            }
        ...
        error => {
            ...
            // Handle other size-related errors
            if (error.message && (
