...
class ShardFunctions {
  ...
  static calculateShardGlobals(numNodes: number, nodesPerConsenusGroup: number, nodesPerEdge: number): ShardGlobals {
    ...
    if (nodesPerConsenusGroup % 2 === 0) {
      ...
      if (config.VERBOSE) console.log('upgrading consensus size to odd number: ' + nodesPerConsenusGroup)
    }
  ...
  static calculatePartitionRangeInternal(
    ...
    if (wrappablePartitionRange.partitionStart < 0) {
      ...
      wrappablePartitionRange.partitionStart2 = wrappablePartitionRange.partitionStart + shardGlobals.numPartitions
    ...
    if (wrappablePartitionRange.partitionEnd >= shardGlobals.numPartitions) {
      wrappablePartitionRange.rangeIsSplit = true
      wrappablePartitionRange.partitionEnd1 = wrappablePartitionRange.partitionEnd - shardGlobals.numPartitions
      wrappablePartitionRange.partitionStart1 = 0
    ...
    if (
      ...
      throw new Error(
        'this should never happen: ' +
          StringUtils.safeStringify(wrappablePartitionRange) +
          'globals: ' +
          StringUtils.safeStringify(shardGlobals)
      )
    }
  ...
  static computeNodePartitionDataMap(
    ...
    nodesToGenerate: P2P.NodeListTypes.Node[], //P2P.NodeListTypes.Node[],
    partitionShardDataMap: PartitionShardDataMap,
  ...
  static nodeSortAsc(a: P2P.NodeListTypes.Node, b: P2P.NodeListTypes.Node): Ordering {
  ...
  //   static dilateNeighborCoverage (shardGlobals: ShardGlobals, nodeShardDataMap: Map<string, NodeShardData>, parititionShardDataMap: Map<number, ShardInfo>, activeNodes: P2P.NodeListTypes.Node[], nodeShardDataToModify:NodeShardData, extras: P2P.NodeListTypes.Node[]) {
   ...
   * @param {P2P.NodeListTypes.Node[]} listA
   * @param {P2P.NodeListTypes.Node[]} listB
   * @returns {array} result [results, extras]  TODO, convert the array response to a type object that has results and extras
   */
  static mergeNodeLists(listA: P2P.NodeListTypes.Node[], listB: P2P.NodeListTypes.Node[]): P2P.NodeListTypes.Node[][] {
    const results = [] as P2P.NodeListTypes.Node[]
    const extras = [] as P2P.NodeListTypes.Node[]
   ...
   * @param {P2P.NodeListTypes.Node[]} listA
   * @param {P2P.NodeListTypes.Node[]} listB
   * @returns {P2P.NodeListTypes.Node[]} results list
   */
  static subtractNodeLists(listA: P2P.NodeListTypes.Node[], listB: P2P.NodeListTypes.Node[]): P2P.NodeListTypes.Node[] {
    const results = [] as P2P.NodeListTypes.Node[]
    const nodeSet = new Set()
    for (const node of listB) {
      nodeSet.add(node)
  ...
  static getCombinedNodeLists(
    ...
    const consensusNodeForOurNode = [] as P2P.NodeListTypes.Node[]
    const consensusNodeForOurNodeFull = [] as P2P.NodeListTypes.Node[]
    const nodeThatStoreOurPartition = [] as P2P.NodeListTypes.Node[]
    const nodeThatStoreOurPartitionFull = [] as P2P.NodeListTypes.Node[]
    const edgeNodes = [] as P2P.NodeListTypes.Node[]
   ...
   * @param {ShardGlobals} shardGlobals
   * @param {P2P.NodeListTypes.Node[]} activeNodes
   * @param {number} position
